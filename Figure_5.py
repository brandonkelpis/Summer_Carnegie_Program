...
# The purpose of this code is to replicate Stuckers Milky Way enclosed mass as a function of radius with M31's parameters.
...
def integrand_NFW (r):
  ...
  b =(1 + (r/rc))**2
  c = a * b
  return (pc / c) * 4 * np.pi *r**2   # Surface area multiplied to return statement to obtain mass from single integral integration.
...
radii = np.geomspace(0.000001,1001,1001) # X-axis. Logarithmic scale.
...
# Cutting off graphs at virial radii.
...
plt.plot(radii[:i_stellar_disk],y2[:i_stellar_disk], label = 'stellar disk', color = 'blue')

plt.plot(radii[:i_bulge],y1[:i_bulge], label = 'bulge', color = 'purple')

plt.plot(radii[:i_baryons],baryon[:i_baryons], label = 'all baryons', color = 'green')

plt.plot(radii[:i_total],y6[:i_total], label = 'total', color = 'orange')

i = np.argmax(radii > 213)

plt.plot (radii[:i], y7[:i], label = 'NFW', color = 'black')
