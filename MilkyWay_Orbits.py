...
#update masses etc....
class MilkyWay():
    def __init__(self, h=0.679, adiabatic_contraction=True, cachedir=None, mass_halo=2e12, conc_initial=7.4,mass_disk=5.6e10,scalelength_disk = 5000,thickness_disk = 1100, mass_bulge=3.3e10,scalelength_bulge= 55e1, mode="cautun_2020"):
        """
        ...
        self.par["stardisk"] = dict(mass = mass_disk, scalelength = scalelength_disk, thickness = thickness_disk)
        self.par["gasdisk"]  = dict(mass = 0, scalelength=7e3, thickness = 8e1) # related directly to b and a #plug these bitches into the argument. then go to first doc 
        self.par["bulge"]  = dict(mass = mass_bulge, scalelength= scalelength_bulge)
        ...
        if adiabatic_contraction:
            ...
            rbins = np.logspace(-6, 1, 1000) * 213e3           #adjusting based of ratios....lolol... was 200e^3 the virial radius for halo of mw? if so my new value is correct.
    ...
    def create_dm_orbits(self, ntot=10000, nsteps=10000, tmax=1e10, rmax=650e3, components="hbs", seed=42, addinfo=False, subsamp=1, adaptive=False, verbose=False, mpicomm=None):
